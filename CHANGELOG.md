...
## 2026-05-16 — 课件 L21-22 应对与写作模块补齐

- 课件纯文字版 L21-22（计量综述写作）阅读评估，识别 8 项写作过程产出要求
- 新增 8 个写作过程文件：综述类型声明、RQ、引言骨架、方法初稿（10项透明清单）、RQ-方法-图表映射、证据链表、结果段落初稿（含图注7要素）
- manuscript §2.1.1 双数据集口径统一：643（宏观格局）vs 147（CiteSpace图谱+Python分析）
- 质量基准/课程要求细则 新增 L21-22 章节
- 分析报告 v2 新增 L21-22 写作对照评估（89/100）
- QC Checklist 新增写作质量 18 项自检
