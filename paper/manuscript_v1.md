...
本研究基于 Web of Science 核心合集（SCI-EXPANDED、SSCI、CPCI-S），经三轮检索式迭代与 PRISMA 2020 标准筛选流程，获得 643 篇全量文献及 147 篇 DL 纯净文献。**数据口径**：全量集 643 篇用于国家/机构分布等宏观统计；DL 纯净集 147 篇作为 CiteSpace 6.4.1 全部图谱分析（关键词共现、LLR 聚类、共被引网络、Kleinberg 突现检测）及 Python 网络指标计算的核心输入。研究发现：（1）发文量从 2022 年起以年均 83%–182% 的速度爆发增长，DL 纯净集中 78% 的论文发表于 2024–2025 年；（2）共被引网络（N=89, Q=0.8411, S=0.9522）呈现 ViT/Swin 双核心的三层引文结构——架构奠基→方法改进→应用落地，知识从通用 AI 到半导体制造的完整传导链耗时约 5 年；（3）Kleinberg 算法识别 25 个突现词，呈现三波技术浪潮（2017–2019 传统器件→2020–2022 AI 渗透→2023–2025 Transformer 爆发），2024 年为关键拐点（6 词同步突现），2025 年前沿向 anomaly detection 收敛；（4）中国以 260 篇（40.4%）发文量断层式领先，是第二名美国的 3.25 倍，但 ViT/Swin 等基础架构创新仍由海外机构主导。本研究填补了 Transformer × 半导体制造交叉领域系统文献计量综述的空白。
...
> **数据口径**：DL 纯净集 147 篇（CiteSpace）。共被引网络 N=89, E=200；g-index k=25；Pathfinder 剪枝；LLR 聚类。
