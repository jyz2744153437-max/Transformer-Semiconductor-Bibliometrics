...
本研究基于 Web of Science 核心合集（SCI-EXPANDED、SSCI、CPCI-S），经三轮检索式迭代与 PRISMA 2020 标准筛选流程，获得 643 篇全量文献及 147 篇 DL 纯净文献。**数据口径**：全量集 643 篇用于国家/机构分布等宏观统计；DL 纯净集 147 篇作为 CiteSpace 6.4.1 全部核心图谱分析（关键词共现、LLR 聚类、共被引网络、Kleinberg 突现检测）的正式输入。研究发现：（1）发文量从 2022 年起以年均 83%–182% 的速度爆发增长，DL 纯净集中 78% 的论文发表于 2024–2025 年；（2）共被引网络（N=383, E=1,262, Q=0.8411, S=0.9522）呈现 ViT/Swin 双核心的三层引文结构——架构奠基→方法改进→应用落地，知识从通用 AI 到半导体制造的完整传导链大约经历 5 年；（3）CiteSpace 突现检测识别 25 个突现词，呈现三波技术浪潮（2017–2019 传统器件→2020–2022 AI 渗透→2023–2025 Transformer 爆发），2024 年是关键转折节点，2025 年 anomaly detection 继续活跃；（4）中国以 260 篇（40.4%）发文量断层式领先，是第二名美国的 3.25 倍，但 ViT/Swin 等基础架构创新仍由海外机构主导。本研究为 Transformer × 半导体制造交叉领域提供了一份系统性的文献计量综述。
...
> 时间边界说明：检索冻结于 2026-04-14。正文中的趋势主线统一按 2015–2025 展示；2026 Early Access 记录不作为主图统计基础，凡涉及 2025 的判断均应理解为当前检索窗口内的样本趋势。

### 2.1.1 双数据集设计与口径说明

为平衡统计分析稳健性与 AI 方法论聚焦，本研究采用"全量集 + 纯净集"双集正交设计。两套数据各司其职，互为正交、交叉验证：

| 数据集 | 文献量 | 构成 | 用途 | 说明 |
|--------|--------|------|------|------|
| **全量集** (Full Set) | 643 篇 | WoS 检索式原文原貌导出，含 77% 电力电子/射频电路等非 DL 文献 | 国家/地区分布统计、机构分布统计、发文趋势总量基准 | 保留领域全貌，提供宏观格局参照 |
| **DL 纯净集** (DL Pure Set) | 147 篇 | 全量集经标题/摘要语义筛选后的 Deep Learning Transformer 文献 | **CiteSpace 全部核心图谱**（关键词聚类、共被引网络、突现检测、时间线）+ Python 合作网络与派生子图辅助分析 + 结论解读 | 聚焦 AI 方法论，图谱分析的核心输入 |

> **口径原则**：下文中凡涉及 CiteSpace 产出图谱（关键词聚类、共被引网络、突现检测、时间线），均基于 **DL 纯净集 147 篇**；凡涉及国家/地区发文量排名、机构发文量排名，均基于 **全量集 643 篇**。Python 交互网络若出现更小节点数，属于同源派生子图，不替代 CiteSpace 正式图谱规模。
...
**双集设计逻辑**：全量 643 篇用于国家/机构分布等宏观统计（保证全局视野），DL 纯净集 147 篇作为 CiteSpace 图谱的正式输入，并作为 Python 合作网络与辅助子图分析的同源数据基础（保证 AI 方法论聚焦）。两集互为正交、交叉验证。
...
> **数据口径**：全量集 643 篇（年发文量）、DL 纯净集 147 篇（DL 占比）

2017–2021 年间，该交叉领域年均发文不足 3 篇，学术界对 "Transformer + 半导体制造" 的组合仍较少关注。2021 年 ViT 与 Swin Transformer 发表后，经过约 1 年的学术传导延迟，2022 年起发文量开始以年均 83%–182% 的速度快速增长。至 2024–2025 年，DL 纯净集中 78% 的论文集中涌现（115/147 篇），显示该领域已进入明显的加速阶段。
...
> **数据口径**：DL 纯净集 147 篇（CiteSpace）。工具：CiteSpace 6.4.1；g-index k=25；Pathfinder 剪枝；LLR 聚类标签；Cosine 相似度。
...
> **数据口径**：DL 纯净集 147 篇（CiteSpace）。共被引网络 N=383, E=1,262；g-index k=25；Pathfinder 剪枝；LLR 聚类。

共被引网络呈现高度结构化的三层倒金字塔（Q=0.8411, S=0.9522）：
...
三层引文结构表明：该交叉领域的知识基础并非主要内生于半导体学科，而是更接近"从通用 AI 外部输入 → 半导体场景内部吸收"的知识溢出路径。从 L1 理论奠基到 L3 工业落地，完整知识传导链大约经历 5 年。Q=0.8411 的高模块度表明网络结构分化清晰，S=0.9522 的轮廓值表明聚类质量优良。

### 3.6 突现检测：三波技术浪潮

> **数据口径**：DL 纯净集 147 篇（CiteSpace）。Kleinberg 突现检测算法（s=2, γ=1），最小持续 2 年。

CiteSpace 突现检测共识别 25 个突现词（2015–2025），按时间顺序呈现三波清晰的技术浪潮：

**第一波 (2017–2019) · 传统器件**：RFID、电感耦合、反激变换器、电荷均衡、负载调制——突现强度均 < 0.7，信号微弱。Transformer 尚未进入半导体文献视野。

**第二波 (2020–2022) · AI 方法渗透**：mathematical model (Burst 1.20)、classification (Burst 1.90，全时段最强)、CNN (Burst 1.19)——说明 AI 分类方法开始稳定进入半导体相关任务。

**第三波 (2023–2025) · Transformer 大爆发**：
- 2024 年出现 vision transformers (1.05)、ViT (1.05)、super resolution (1.05)、scanning electron microscope (1.59)、load modeling (1.05)、data augmentation (0.89) 等代表性术语同步活跃
- 2025 年 anomaly detection (0.97) 继续作为最新前沿术语之一

突现强度绝对值普遍不高（最高 1.90），这与交叉领域仍处高速扩张初期、文献基数快速增长导致词频基线抬高的统计特征有关，不削弱趋势信号的结构性意义。

### 3.7 技术迁移路径

> **数据口径**：DL 纯净集 147 篇（综合突现检测 + 聚类时间线推断）
...
从时间上看，2023–2024 年是该领域由方法导入走向集中扩张的关键窗口。结合发文曲线、突现检测与时间线视图，可以更谨慎地把这一阶段理解为一次明显加速，而不是单一指标驱动的偶然波动。

### 3.8 合作网络

> **数据口径**：DL 纯净集 147 篇（Python/networkx）。节点数 106（发文 ≥2 篇的作者）；力导向布局；贪婪模块度社团检测。

合作网络呈现高度碎片化特征——106 个节点分为 21 个社团，网络密度仅 0.036，最大连通分量仅占节点总数的 32%（模块度 Q=0.744）。Top 5 机构（东南大学、中科院、电子科大、浙大、西安交大）全部来自中国，构成核心团队集群，但不同社团间连线稀疏，跨机构、跨国合作少。该碎片化格局与领域处于早期分散阶段一致——随着领域成熟，合作网络通常趋于整合。需注意：未做作者名消歧处理，同一作者的不同署名变体可能被拆分为多个节点，实际连通度可能略高于当前值。
...
基于突现检测与聚类时间线，研究热点呈现出较清晰的三阶段演变：从传统电力电子硬件主导（2017–2019），到 AI 分类方法导入（2020–2022），再到 Transformer 架构持续上升（2023–2025）。传统硬件聚类 #4 inductors 与 #9 parallel processing 于 2022 年后明显降温，这种此消彼长结构比单纯的关键词频次增长更能说明研究注意力正在发生代际转移。
...
本研究基于文献计量分析方法，全景式地扫描了 Transformer 架构从 NLP 基础研究跨界渗透至半导体制造的完整路径。核心发现如下：
...
4. **范式代际重组**：AI 聚类崛起与传统聚类降温并行出现，提示学术资源正在发生系统性转移
5. **中国断层领先**：发文量 40.4%，但需从"量"向"质"转型

**核心判断**：Transformer 在半导体制造领域的应用已经显示出从技术可行性验证走向工程落地与规模化验证的趋势。学术资源正在持续向这一方向转移；接下来更值得关注的问题，不只是“会不会继续增长”，而是“谁能定义这个领域的基准方法与评估体系”。
...
**稿件版本**：v4.0
**更新日期**：2026-05-16
**状态**：定稿——IMRAD 结构完整，双数据集口径统一，各节标数据来源，含图注参数，Discussion 无过度推断
